function createCalibratedMeasurementFigure(actual, ua, beta, Ubeta, dataName, phi, psi, SixSigmaRule)
% createCalibratedMeasurementFigure - Plot confidence regions for calibrated device measurement
%
% This function creates a figure illustrating the calibrated measurement
% considering the uncertainty of the actual measured values.
%
% Inputs:
...
% Settings
sigx = 0.0008;  % standard deviation for device measuring capability

n_phi = norminv(1 - phi/2);
n_psi = norminv(1 - psi/2);

% Select representative point for detailed analysis
% (optional: here we use mid-range point for illustration)
...
% Shift grid by ± uncertainty and calculate new predictions
...
plot(xx, yhat_minus - ci_minus, 'k--', 'DisplayName', 'Lower Bound (Shifted -)')
...
% Save the figure
savefig(sprintf('Fig_%s_CalibratedMeasurement.fig', dataName));
